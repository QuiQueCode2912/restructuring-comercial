...
class IndexController extends Controller
{
    ...
    public function e109(Request $request)
    {
        ...
        $venues = Venue::where('id', '=', '02i3m00000AalSXAAZ')
                        ->where('show_on_website', 'Si')
                        ->get();
    ...
    public function parqueCds(Request $request)
    {
        ...
        $includeIds = ['02i3m00000Didu7AAB', '02i3m00000Fx0PEAAZ', '02i3m00000Fx0PJAAZ', '02i3m00000DidtxAAB', '02i3m00000Didu3AAB', 
                        '02i3m00000FbwwjAAB', '02i3m00000DiduZAAR','02i3m00000DiduCAAR','02i3m00000DiduVAAR','02iRb000000N4KfIAK','02i3m00000DidtdAAB','02i3m00000Didu2AAB'];
            ...
            $venues = Venue::where('parent_id', '=', $parent->id)
                ...
                ->where(function ($query) {
                    $query->where('id', '=', '02i3m00000Didu7AAB')
                        ...
                        ->orWhere('id', '=', '02i3m00000DidtdAAB')
                        ->orWhere('id', '=', '02i3m00000Didu2AAB')
        ...
        switch ($step) {
            case 'datos-contacto':
                ...
                if (isset($_COOKIE['first_name'])) {
                    session(['first_name' => $_COOKIE['first_name']]);
                }
                if (isset($_COOKIE['last_name'])) {
                    session(['last_name' => $_COOKIE['last_name']]);
                }
                if (isset($_COOKIE['email'])) {
                    session(['email' => $_COOKIE['email']]);
                }
                if (isset($_COOKIE['phone'])) {
                    session(['phone' => $_COOKIE['phone']]);
                }
                if (isset($_COOKIE['company'])) {
                    session(['company' => $_COOKIE['company']]);
                }
                if (isset($_COOKIE['00N3m00000QQOde'])) {
                ...
                if ($request->isMethod('post')) {
                    ...
                    if((
                        ...
                        && $request->email != 'three3brains@gmail.com'
                    ...
                    case '02i3m00000D9DaPAAV':
                        ...
                        switch (session()->get('franja')) {
                            ...
                            case ($venueId == '02i3m00000D9GuYAAV'):
                                $venuesgrupo = Venue::where('parent_id', '=', '02i3m00000DiduCAAR')->where('show_on_website', '=', 'No')->where('hour_fee', '>', 0)->where('showInCalendar', '!=', 'No')->orderBy('name', 'asc')->get();
                                break;
                            default:
            ...
            case 'vista-previa':
                ...
                $form_url = 'https://test.salesforce.com/servlet/servlet.WebToLead?encoding=UTF-8&orgId=00DO8000003Y6Iz';
    ...
    public function getAvailableSlots(Request $request)
    {
        ...
        foreach ($events['records'] as $event) {
            ...
            if (
                isset($event['Venue__r']['Bloqueo_adicional_1__c']) ||
